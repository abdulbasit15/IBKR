...
TIER-2 MEAN-REVERSION (`chop_action:"mean_revert"`, the third CHOP option): instead of merely
sitting out a CHOP regime, actively FADE band extremes back toward the mean — the trend-follower's
"defence" in chop becomes "offence". It's a distinct, counter-trend sub-strategy (its side comes
from the Bollinger/RSI extreme, NOT the Supertrend): go LONG when price is at/below the lower
Bollinger band (percent_b <= entry_pb) and RSI is oversold, SHORT when at/above the upper band and
RSI is overbought (RSI confirmation optional; `direction` is respected so a long_only strategy only
fades longs). The stop is FIXED at stop_atr_mult*ATR beyond the band (never trailed to the
Supertrend), and the trade exits on reversion to the Bollinger basis (take-profit), on the stop, on
a regime flip back to TREND (the range thesis is broken), or on a max_hold_bars time-stop. Trend
entries stand aside while in CHOP; when the regime is TREND the bot resumes its normal Supertrend
entries. Config lives under `regime_filter.mean_revert` (see below). Ships disabled.
...
# This bot may sit directly under <Trading Strategies> OR nested deeper (e.g. this v2 lives in
# Indicator Strategies/supertrendv2/), so rather than assume a fixed depth we WALK UP from the bot
# dir until we find the folder that CONTAINS the Indicators package and add that to sys.path — that
# keeps `from Indicators...` working from source at any nesting. When frozen the package is bundled
# into the exe (the build passes --collect-submodules Indicators).
_BOT_DIR = os.path.dirname(os.path.abspath(__file__))


def _find_shared_root(start: str) -> str:
    d = start
    for _ in range(8):
        if os.path.isdir(os.path.join(d, "Indicators")):
            return d
...
_SHARED_ROOT = _find_shared_root(_BOT_DIR)        # ...\Trading Strategies
...
from Indicators.volatility.bollinger_bands import bollinger_value  # noqa: E402
from Indicators.volatility.atr import atr_value            # noqa: E402
...
class SupertrendBot:
    def __init__(self, cfg: dict, base_dir: str):
        ...
        self.regime_chop_action = ca if ca in ("stand_aside", "momentum", "mean_revert") else "stand_aside"
        self.regime_trend_momentum = bool(rgm.get("trend_momentum", False))
        self._regime: dict[str, str] = {}   # symbol -> "TREND"/"CHOP" (hysteresis state)

        # TIER-2 mean-reversion sub-strategy (only active when regime_filter is enabled AND
        # chop_action="mean_revert"). In a CHOP regime it FADES Bollinger-band extremes back to
        # the mean: LONG at/below the lower band + RSI oversold, SHORT at/above the upper band +
        # RSI overbought (RSI optional). The stop is FIXED stop_atr_mult*ATR beyond the band; the
        # take-profit is the Bollinger basis. Counter-trend, so its side is independent of the
        # Supertrend. Config: "regime_filter": { ..., "chop_action": "mean_revert", "mean_revert": {
        #   "bb_length": 20, "bb_mult": 2.0, "entry_pb": 0.05, "require_rsi": true, "rsi_period": 14,
        #   "rsi_oversold": 30, "rsi_overbought": 70, "atr_period": 14, "stop_atr_mult": 1.0,
        #   "max_hold_bars": 12 } }
        mr = rgm.get("mean_revert", {})
        if not isinstance(mr, dict):
            mr = {}
        self.mean_revert_enabled = self.regime_enabled and self.regime_chop_action == "mean_revert"
        self.mr_bb_length = int(mr.get("bb_length", 20))
        self.mr_bb_mult = float(mr.get("bb_mult", 2.0))
        self.mr_entry_pb = float(mr.get("entry_pb", 0.05))
        self.mr_require_rsi = bool(mr.get("require_rsi", True))
        self.mr_rsi_period = int(mr.get("rsi_period", 14))
        self.mr_rsi_os = float(mr.get("rsi_oversold", 30.0))
        self.mr_rsi_ob = float(mr.get("rsi_overbought", 70.0))
        self.mr_atr_period = int(mr.get("atr_period", 14))
        self.mr_stop_atr_mult = float(mr.get("stop_atr_mult", 1.0))
        self.mr_max_hold_bars = int(mr.get("max_hold_bars", 12))
    ...
    def _derive_hist_duration(self) -> str:
        ...
        needed = max(self.atr_period,
                     ...
                     self.regime_adx_period if self.regime_enabled else 0,
                     self.regime_chop_period if self.regime_enabled else 0,
                     self.mr_bb_length if self.mean_revert_enabled else 0,
                     self.mr_atr_period if self.mean_revert_enabled else 0,
                     self.mr_rsi_period if self.mean_revert_enabled else 0)
    ...
    def regime_gate_ok(self, symbol, bars, side) -> bool:
        ...
        if regime == "CHOP":
            if self.regime_chop_action in ("stand_aside", "mean_revert"):
                # stand_aside -> no entries. mean_revert -> TREND-following entries stand aside in
                # chop (the counter-trend fade is handled separately in maybe_open_mean_revert);
                # either way the Supertrend entry is blocked here.
                self.log(f"{symbol} regime CHOP ({ctxt}/{atxt}) -> stand aside, no {side} trend entry")
                return False
    ...
    def maybe_open_mean_revert(self, symbol, bars, contract, close, tick, bar_time) -> bool:
        """TIER-2 entry (CHOP regime only): FADE a Bollinger-band extreme back toward the mean.
        LONG when price is at/below the lower band (percent_b <= entry_pb) and RSI is oversold;
        SHORT when at/above the upper band and RSI is overbought (RSI confirmation optional via
        require_rsi). The protective stop sits stop_atr_mult*ATR BEYOND the band and the
        take-profit target is the Bollinger basis (the mean). This is counter-trend, so the side
        comes from the band/RSI, NOT the Supertrend, but the `direction` config is still honoured
        (a long_only strategy only takes long fades). Returns True if a fade was opened. A filter
        that can't be evaluated (insufficient history) blocks the fade — same convention as the
        trend filters."""
        bres = bollinger_value(symbol=symbol, bar_size=self.bar_size,
                               length=self.mr_bb_length, mult=self.mr_bb_mult, bars=bars)
        if bres is None:
            self.log(f"{symbol} mean-revert: insufficient bars for BB({self.mr_bb_length}) "
                     f"(have {len(bars)}); no fade")
            return False
        pb = bres.percent_b
        rres = rsi_value(symbol=symbol, bar_size=self.bar_size, period=self.mr_rsi_period, bars=bars)
        rsi_v = rres.value if rres else None
        if self.mr_require_rsi and rsi_v is None:
            self.log(f"{symbol} mean-revert: insufficient bars for RSI({self.mr_rsi_period}); no fade")
            return False

        side = None
        if pb <= self.mr_entry_pb and self.allow_long:
            if not self.mr_require_rsi or (rsi_v is not None and rsi_v <= self.mr_rsi_os):
                side = LONG
        elif pb >= (1.0 - self.mr_entry_pb) and self.allow_short:
            if not self.mr_require_rsi or (rsi_v is not None and rsi_v >= self.mr_rsi_ob):
                side = SHORT
        if side is None:
            return False

        ares = atr_value(symbol=symbol, bar_size=self.bar_size, period=self.mr_atr_period, bars=bars)
        if ares is None:
            self.log(f"{symbol} mean-revert: insufficient bars for ATR({self.mr_atr_period}); no fade")
            return False
        atrv = ares.value
        stop = (bres.lower - self.mr_stop_atr_mult * atrv) if side == LONG \
            else (bres.upper + self.mr_stop_atr_mult * atrv)
        target = bres.basis
        # the basis must be on the PROFIT side of the entry, else the setup is degenerate (price
        # already through the mean) — skip rather than open a trade with no room to revert.
        if (side == LONG and target <= close) or (side == SHORT and target >= close):
            self.log(f"{symbol} mean-revert: basis {target:.2f} not beyond entry {close:.2f} "
                     f"for {side}; skip")
            return False

        rtxt = f"RSI {rsi_v:.0f}" if rsi_v is not None else "RSI off"
        self.log(f"{symbol} MEAN-REVERT {side} fade: %B {pb:.2f} {rtxt} "
                 f"band[{bres.lower:.2f},{bres.upper:.2f}] basis {target:.2f} "
                 f"ATR {atrv:.2f} stop {stop:.2f}")
        # reuse the shared entry mechanics (sizing, atomic entry+stop bracket, chase, overnight):
        # passing `stop` as the structural stop line makes open_position build the fixed stop there.
        self.open_position(symbol, side, contract, stop, close, tick, bar_time)
        # tag the freshly opened position as a mean-revert trade with its take-profit + time-stop
        # bookkeeping, so manage_mean_revert (not the trend manager) handles it from here.
        p = self.positions.get(symbol)
        if p and p.get("side") == side:
            p["strategy"] = "mean_revert"
            p["target"] = target
            p["mr_bars"] = 0
            p["mr_last_bar"] = bar_time
            return True
        return False

    def manage_mean_revert(self, symbol, p, bars, contract, tick, close, bar_time):
        """Manage an open TIER-2 mean-reversion (fade) position. Unlike a trend position, the stop
        is FIXED (never trailed toward the Supertrend) and the trade exits on reversion to the
        mean. Exit priority: (1) protective stop hit (native fill or synthetic breach), (2) regime
        flipped back to TREND (the range thesis is broken), (3) price reverted to the Bollinger
        basis — take profit, (4) max_hold_bars time-stop. Otherwise keep the fixed stop live on the
        active venue. Exits are never gated (this runs every bar regardless of the entry window)."""
        oc = self._order_contract(contract)
        overnight = self._is_overnight(oc)
        last_px = bars[-1].close if bars else close
        st = p.get("st")

        # advance the held-bar counter once per NEW completed bar (for the time-stop)
        if bar_time != p.get("mr_last_bar"):
            if p.get("mr_last_bar") is not None:
                p["mr_bars"] = int(p.get("mr_bars", 0)) + 1
            p["mr_last_bar"] = bar_time

        def _exit(reason):
            """Flatten and close on a full fill; on a partial keep the (reduced) position for the
            next bar. Returns True if the position is now closed."""
            self.cancel(st)
            ...
            if self._flatten_complete(tr, p["qty"]):
                ...
                self.close_position(symbol, exit_px, reason)
                return True
            self._shrink_to_fill(p, tr)
            return False

        # 1) protective stop hit — native server-side fill, or (OVERNIGHT venue) synthetic breach
        ...
        if native_filled:
            self.close_position(symbol, float(st.orderStatus.avgFillPrice or p["stop"]), "MR_STOP")
            return
        if synth_hit:
            self.log(f"{symbol} MR synthetic stop hit: {p['side']} px {last_px:.2f} "
                     f"vs stop {p['stop']:.2f} -> flattening")
            _exit("MR_STOP")
            return

        # 2) regime flipped back to TREND -> the range/fade thesis is broken; get out
        regime, chop, adx = self.current_regime(symbol, bars)
        if regime == "TREND":
            self.log(f"{symbol} MR regime flipped TREND (CHOP {chop:.0f}/ADX {adx:.0f}) "
                     if chop is not None and adx is not None
                     else f"{symbol} MR regime flipped TREND ")
            _exit("MR_REGIME")
            return

        # 3) reverted to the mean (Bollinger basis) -> take profit
        bres = bollinger_value(symbol=symbol, bar_size=self.bar_size,
                               length=self.mr_bb_length, mult=self.mr_bb_mult, bars=bars)
        basis = bres.basis if bres else p.get("target")
        reverted = basis is not None and (
            (p["side"] == LONG and close >= basis) or (p["side"] == SHORT and close <= basis))
        if reverted:
            self.log(f"{symbol} MR reverted to mean {basis:.2f} (close {close:.2f}) -> take profit")
            _exit("MR_TARGET")
            return

        # 4) time-stop -> the fade didn't work within the window; cut it
        if int(p.get("mr_bars", 0)) >= self.mr_max_hold_bars:
            self.log(f"{symbol} MR time-stop: held {p['mr_bars']} bars >= {self.mr_max_hold_bars} "
                     f"-> flattening")
            _exit("MR_TIME")
            return

        # 5) still in the trade -> keep the FIXED stop live on the active venue (NEVER trailed).
        if overnight:
            if st is not None:
                self.log(f"{symbol} entering OVERNIGHT -> switching MR to synthetic stop")
            ...
            if not alive or not same_venue:
                ...
                self.log(f"{symbol} MR protective stop {why} -> re-placing")
                p["st"] = self.reconcile_stops(symbol, oc, p["side"], p["qty"], p["stop"], tick)
    ...
    def manage_symbol(self, symbol):
        ...
        if self._last_log_bar.get(symbol) != bar_time:
            ...
            if self.regime_enabled:
                ...
                if self.mean_revert_enabled:
                    bbres = bollinger_value(symbol=symbol, bar_size=self.bar_size,
                                            length=self.mr_bb_length, mult=self.mr_bb_mult, bars=bars)
                    gtxt += f" MR %B {bbres.percent_b:.2f}" if bbres else " MR %B n/a"
            else:
                gtxt = "REGIME off"
        ...
        if p:
            # TIER-2: a mean-reversion (fade) position is managed by its OWN rules (fixed stop,
            # revert-to-mean take-profit, regime-flip + time exits) — never the trend manager below.
            if p.get("strategy") == "mean_revert":
                self.manage_mean_revert(symbol, p, bars, contract, tick, close, bar_time)
                return
            oc = self._order_contract(contract)          # active venue (OVERNIGHT during 20-04)
            overnight = self._is_overnight(oc)
        ...
        # flat now -> consider a NEW entry. These guards apply to BOTH the trend-following entry
        # and the tier-2 mean-reversion fade, so they run before either branch.
        ...
        # TIER-2 mean-reversion: when regime_filter is on with chop_action="mean_revert", a CHOP
        # regime FADES band extremes back to the mean instead of standing aside. The fade is
        # counter-trend, so it's evaluated independently of the Supertrend `desired` side; while in
        # CHOP we take ONLY the fade (no trend entry). In a TREND regime we fall through to the
        # normal Supertrend entry below.
        if self.mean_revert_enabled:
            regime, _, _ = self.current_regime(symbol, bars)
            if regime == "CHOP":
                self.maybe_open_mean_revert(symbol, bars, contract, close, tick, bar_time)
                return

        # trend-following entry: only meaningful when the Supertrend gives a tradable side.
        if desired == FLAT:
            return
    ...
    def run(self):
        ...
        try:
            ...
            self.log(f"direction={self.direction} symbols={self.symbols} "
                     ...
                     f"REGIME={'on(' + self.regime_chop_action + ')' if self.regime_enabled else 'off'}"
                     f"{'+MEAN_REVERT' if self.mean_revert_enabled else ''} "
