...
def high_impact_events(dates, country: str = "United States") -> list[dict]:
    """All HIGH-impact ('red folder') events over the given dates (Nasdaq).

    Impact is inferred by event name (CPI, PCE, PPI, GDP, Payrolls, FOMC/Fed
    rate, Retail Sales, ISM, Unemployment, Fed Chair). Deduped by (date, event).
    """
    # Nowcasts / regional gauges are not red-folder releases — drop them.
    EXCLUDE = ("gdpnow", "nowcast", "cleveland", "atlanta fed")
    out, seen = [], set()
    for d in dates:
        for r in nasdaq_events(d):
            if r.get("country") != country:
            ...
            if any(x in name.lower() for x in EXCLUDE):
                continue
            m = _match_rule(name)
            if not m or m[1] != "High":
                continue
            label = m[0]                      # canonical headline (CPI, PPI, ...)
            key = (d, label)
            if key in seen:                   # collapse sub-variants to one row
                continue
            seen.add(key)
            out.append({
                "date": d, "time": _clean(r.get("gmt")), "ccy": "USD",
                "impact": "High", "event": label, "detail": name,
                "actual": _clean(r.get("actual")),
                "forecast": _clean(r.get("consensus")),
                "previous": _clean(r.get("previous")),
            })
...
def high_impact_by_day(dates, country: str = "United States") -> dict:
    """{date: [high-impact events]} for the given dates."""
    out = {}
    for e in high_impact_events(dates, country):
