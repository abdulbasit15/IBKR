...
def intraday_bars(asset: str, d, interval: str = "5m") -> list[list]:
    ...
    # Widen the window so every trade on this day has ~4h of bars before entry
    # and ~2h after exit, even for just-after-midnight or late-evening trades.
    day0 = et_epoch(datetime(d.year, d.month, d.day, 0, 0))
    start = day0 - 6 * 3600                        # prev day 18:00 ET
    end = day0 + 30 * 3600                         # next day 06:00 ET
    url = (config.YAHOO_CHART_URL.format(ticker=ticker)
           + f"?period1={start - 3600}&period2={end + 3600}&interval={interval}")
    data = _http_json(url)
    bars: list[list] = []
    try:
        res = data["chart"]["result"][0]
...
_series_cache: dict[tuple, list] = {}


def fetch_series(ticker: str, interval: str = "5m", range_: str = "1mo") -> list[list]:
    """Fetch an OHLC series for a ticker/interval. Returns [[epoch,o,h,l,c],...]."""
    ...
    try:
        ...
        pass
    _series_cache[key] = bars
    return bars


def series_for(asset: str, interval: str, range_: str = "1mo") -> list[list]:
    ticker = config.YAHOO_MAP.get(asset.upper())
    return fetch_series(ticker, interval, range_) if ticker else []


# --- Volume-bearing series (for VWAP / VPOC / RVOL / est. delta) ----------
_vol_cache: dict[tuple, list] = {}


def fetch_series_v(ticker: str, interval: str = "5m", range_: str = "1mo") -> list[list]:
    """OHLCV series: [[epoch, o, h, l, c, v], ...]. Volume kept (may be 0/None)."""
    key = (ticker, interval, range_)
...
def series_v_for(asset: str, interval: str = "5m", range_: str = "1mo") -> list[list]:
    ticker = config.YAHOO_MAP.get(asset.upper())
    return fetch_series_v(ticker, interval, range_) if ticker else []
