...
from . import config, tags as tags_mod
...
def _by_tag(trades) -> list[dict]:
    """Aggregate P&L by individual tag across setup / mistakes / custom columns."""
    agg = defaultdict(lambda: {"pnl": 0.0, "n": 0, "wins": 0, "cat": None})
    for t in trades:
        p = _pnl(t)
        if p is None:
            continue
        seen = set()
        for field in tags_mod.FIELD_KEYS:
            for tg in tags_mod.split(getattr(t, field, "")):
                if tg in seen:
                    continue
                seen.add(tg)
                a = agg[tg]
                ...
                a["cat"] = tags_mod.FIELD_CATEGORY.get(field, "Other")
    rows = [{"tag": k, "category": v["cat"], "n": v["n"],
             "win_rate": v["wins"] / v["n"], "total_pnl": round(v["pnl"], 2),
             "expectancy": round(v["pnl"] / v["n"], 2)}
