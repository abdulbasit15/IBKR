...
# An open position idle longer than this between fills = a stale prior-session
# leftover, not part of the next trade. Scalps last minutes, never hours.
_GAP_RESET_SEC = 4 * 3600
    ...
    for root, grp in df.groupby("root"):
        ...
        for _, o in filled.iterrows():
            ...
            if px is None or qty <= 0:
                continue
            # Stale carry-over guard: an open position followed by a long gap
            # (e.g. an unclosed overnight leftover before the next session) would
            # otherwise thread into the new session's fills and mislabel them.
            # Drop it so the next session reconstructs from flat.
            if (pos != 0 and prev_t is not None and o["t"] is not None
                    and (o["t"] - prev_t).total_seconds() >= _GAP_RESET_SEC):
                pos = 0.0
                direction = None
                en = eq = xn = xq = 0.0
            prev_t = o["t"]
