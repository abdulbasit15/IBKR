...
from . import analytics, autotag, config, excel_io, indicators, market
from . import news as news_mod
from . import tags as tagvocab
...
def _trade_dict(t) -> dict:
    ...
    return {
        ...
        "mistakes": t.mistakes or "", "rules": t.rules or "",
        "market": t.market or "", "emotion": t.emotion or "",
        "process": t.process or "", "learning": t.learning or "",
        "chart": t.trade_screenshot or "", "level": t.instrument_level or "",
    }


def build_data(trades, ai_text: str = "", with_intraday: bool = True,
               tag_vocab: dict | None = None, rules: dict | None = None) -> dict:
    ...
    for i, t in enumerate(trades):
        if labels.get(i) and (not t.setup or t.setup in tagvocab.AUTO_SETUP_LABELS):
            t.setup = labels[i]
            auto_flag[i] = True

    analytics._ensure_derived(trades)             # pnl/rr for mistake detection
    mistakes = autotag.detect_mistakes(trades)
    for i, t in enumerate(trades):
        if not t.mistakes and mistakes.get(i):
            t.mistakes = ", ".join(mistakes[i])

    metrics = analytics.compute_metrics(trades)   # mutates: fills pnl/rr, by_tag
    s = metrics["summary"]
    daily = _daily(trades)
    ...
    snaps = indicators.summarize(trades) if with_intraday else {}
    by_indicator = indicators.performance(trades, snaps) if snaps else []
    tdicts = []
    for i, t in enumerate(trades):
        d = _trade_dict(t)
        d["id"] = i
        ...
        d["ind"] = snaps.get(i)
        d["ind_ai"] = indicators.analysis(t, snaps.get(i), by_indicator,
                                          metrics["by_setup"])
        tdicts.append(d)
    ...
    return {
        ...
        "by_tag": metrics["by_tag"],
        "by_indicator": by_indicator,
        "tag_vocab": tag_vocab or tagvocab.TAXONOMY,
        "tag_fields": tagvocab.CATEGORY_FIELD,
        "rules": rules or {"hard": list(tagvocab.DEFAULT_HARD_RULES),
                           "other": list(tagvocab.DEFAULT_OTHER_RULES)},
...
def build_app(workbook: Path | None = None, ai_text: str = "",
              out: Path | None = None, server: bool = False) -> Path:
    trades = excel_io.read_trades(workbook)
    if not trades:
        raise SystemExit("No trades found in the workbook.")
    vocab = excel_io.read_tag_options(workbook or config.WORKBOOK)
    rules = excel_io.read_rules(workbook or config.WORKBOOK)
    data = build_data(trades, ai_text, tag_vocab=vocab, rules=rules)
