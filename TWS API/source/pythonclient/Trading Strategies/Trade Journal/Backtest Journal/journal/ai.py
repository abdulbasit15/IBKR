...
SYSTEM_PROMPT = (
    ...
    "You also get `by_tag`: performance per journal tag (Setups, Mistakes, "
    "Emotions, Market Conditions, Process), and `indicator_performance`: win "
    "rate and P&L grouped by the technical-indicator state at entry across "
    "1H/15m/5m — Supertrend alignment, 1H trend agreement, 5m EMA cross, MACD, "
    "RSI zone, ADX regime (chop <20 / trend >=25), side of VWAP, and RVOL "
    "(relative volume). These are index/commodity FUTURES (MNQ/MGC etc.); VWAP "
    "side, ADX regime, and RVOL are especially important intraday, so call out "
    "which conditions help or hurt and weigh them heavily.\n\n"
    "Write a focused, honest, actionable review. Be specific and quantitative — "
    "cite the trader's own numbers and tags (e.g. 'trades tagged Revenge trade "
    "averaged -$X'). Do NOT give generic advice. Answer directly in markdown (do "
    "not use any tools) with these sections:\n"
    "## Overall Assessment\n"
    "## What's Working\n"
...
def _payload(metrics: dict, market: dict, news: dict, recent: list) -> dict:
    ...
    return {
        ...
        "indicator_performance": metrics.get("by_indicator", []),
...
def available() -> bool:
    """True if any Claude backend (logged-in CLI or API key) can be used."""
    return active_backend()[0] != "none"
...
# --- Generic ask (custom system prompt) — used by the backtest journal ------
def ask(prompt: str, system: str) -> str:
    """Send a prompt with a custom system prompt via whichever backend is live."""
    ...
    if backend == "cli":
        ...
        if proc.returncode != 0 or not out:
            err = ((proc.stderr or "").strip() or out or "no output")[:400]
            hint = " — run `claude` and /login, then retry." if "log" in err.lower() else ""
            return f"[AI error via claude CLI] {err}{hint}"
        return out
    if backend == "api":
        key = _api_key()
        body = json.dumps({
            "model": config.ANTHROPIC_MODEL, "max_tokens": config.AI_MAX_TOKENS,
        ...
        return "\n".join(p for p in parts if p).strip() or "[AI returned no text]"
    return "[AI skipped] No Claude backend available."
